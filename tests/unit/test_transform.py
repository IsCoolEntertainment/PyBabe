...
class TestMapTo(TestCase):
    def test_tuple(self):
        a = Babe().pull(filename='tests/files/test.csv', name='Test').typedetect()
...
"""
    ...
    def test_insert(self):
        a = Babe().pull(filename='tests/files/test.csv', name='Test').typedetect()
        a = a.mapTo(lambda row: row.foo + 1,
                    insert_fields=['fooplus'])
...
class TestFilter(TestCase):
    ...
    #    a = Babe().pull(stream=StringIO('a,b\n1,2\n3,4\n1,4\n'), format="csv").typedetect()
    #    a = a.groupBy(key="a",
    #                  reducer=lambda rows: rows + rows[0].\
    #                    _replace(b=sum([row.b for row in rows])))
    #    buf = StringIO()
